const log = require('debug')('chat:controller:chat');
const elog = require('debug')('chat:error:controller:chat');
...
exports.newMessage = async (io, socket, data, callback) => {
	...
	try {
		...
		if (valid.error) {
			...
			const result = await kafka.produce(config.TOPICS.API, 'newMessage', msg);
...
exports.readMessage = async (io, socket, data, callback) => {
	...
	try {
		...
		if (valid.error) {
			...
			const sender_sockets = await user_dao.getUserSocketsById(sender_id);
			const result = await kafka.produce(config.TOPICS.API, 'readMessage', {
				sender_id,
				receiver_id,
				room_id,
				message_id
