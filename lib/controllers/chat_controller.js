	...
	try {
		...
		producer.send([Object.assign(config.PAYLOAD, {
			...
			if (e) {
				...
				kafka_log(result);
				io.to(room_id).emit('send_message', msg);
			}
		});
...
exports.isTyping = async (io, socket, data) => {
	...
	try {
		const sender_id = await chat_dao.getSenderIdBySocket(room_id, socket.id);
		if (typing) {
			io.to(room_id).emit('start_typing', {
				sender_id: sender_id
			});
		} else
			io.to(room_id).emit('stop_typing', {
				sender_id: sender_id
			});
	} catch (e) {
...
exports.readMessage = async (io, socket, data) => {
	...
	try {
		...
		const keyedMessage = new km('readMessage', JSON.stringify(msg));
		producer.send([Object.assign(config.PAYLOAD, {
			messages: [keyedMessage]
		})], function (e, result) {
			if (e) {
				...
				for(const sender_socket of sender_sockets){
					io.to(sender_socket).emit('read_message', msg);
				}
