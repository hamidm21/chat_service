...
exports.createRoom = async (io, socket, data, callback) => {
	...
	try {
		...
		if (valid.error) {
			...
			const notifyKeyedMessage = new km('newMessage', JSON.stringify(lastMessage[0]));
			const autoKeyedMessage = new km('newMessage', JSON.stringify(lastMessage[1]));
...
exports.disconnecting = async (io, socket) => {
	...
	try {
		...
		default: {
			const rooms = await user_dao.getSocketUniqueRooms(socket.handshake.query.user_id, socket.id);
			log({rooms});
			await room_dao.removeSocketFromRooms(rooms, socket.id);
...
exports.choosePsychologist = async (io, socket, data, callback) => {
	...
	try {
		...
		if (valid.error) {
			...
			const keyedMessage = new km('choosePsychologist', JSON.stringify({
				room_id,
				psychologist_id
			}));
			producer.send([Object.assign({}, config.PAYLOAD, {
...
exports.expireTransaction = async (io, socket, data, callback) => {
	...
	try {
		...
		if (valid.error) {
			...
			const keyedMessage = new km('expireTransaction', JSON.stringify({
				room_id,
				user_id
			}));
			producer.send([Object.assign({}, config.PAYLOAD, {
