...
exports.joinSocketToRoom = async (room_id, socket_id, joined_id)=> {
	try{
		const saved = await client.hmsetAsync(room_id, [socket_id, joined_id]);
		if(saved){
			redis_log(saved);
			const members = await client.hlenAsync(room_id);
			if(members !== 0){
				const roomMembers = await client.hvalsAsync(room_id);
...
exports.removeSocketFromRoom = async (rooms , socket_id)=> {
	try{
		...
		return Promise.resolve({sender_id: sender_id , rooms: rooms});
